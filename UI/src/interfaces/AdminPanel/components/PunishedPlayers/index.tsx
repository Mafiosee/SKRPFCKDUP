...
import { Punishment } from '../../../Hud/components/Punishment'
...
const Categories = [
  PunishedTypes.banned,
  PunishedTypes.muted,
  PunishedTypes.demorgan,
]
...
export const PunishedPlayers: React.FC = () => {
  ...
  const onClickRemovePunishment = (id: number) => {
    const payload: AdminPanelPayloads[AdminPanelEvents.RemovePunishment] = {
      id,
    }
  ...
  const searchResults = SearchFields.reduce<PunishedPlayerType[]>(
    (acc, field) => {
  ...
  const onClickArrow = (step: StepTypes) => {
    let newPunished
    if (step === StepTypes.Left) {
      newPunished = currentPunished - 1
      if (newPunished < 0) {
        newPunished = Categories.length - 1
      }
    } else if (step === StepTypes.Right) {
      let newPunished = currentPunished + 1
      if (newPunished > Categories.length - 1) {
        newPunished = 0
      }
    }
    setCurrentPunished(newPunished)
  }
            ...
            {punishedPlayers && (
              <>
                ...
                <div className="content">
                  <div className="blocks">
                    <div className="block">
                      ...
                      <div className="text">
                        {punishedInfo
                          ? PunishedNames[Categories[currentPunished]]
                          : 'Текст'}
                      </div>
                    </div>
                    <div className="block">
                      ...
                      <div className="text">
                        {punishedInfo ? punishedInfo.date : 'Текст'}
                      </div>
                    </div>
                    <div className="block">
                    ...
                    <div className={`block reason`}>
                      ...
                      <div className="text">
                        {punishedInfo ? punishedInfo.reason : ''}
                      </div>
                    </div>
                    ...
                    <div className="block">
                      ...
                      <div className="text">
                        {punishedInfo ? punishedInfo.endPunished : ''}
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    className={`btn`}
                    onClick={() => onClickRemovePunishment(punishedInfo.id)}
                  >
                ...
                <div className="block-info">
                  <div className="name">
                    {punishedInfo
                      ? PopupPunishedNames[Categories[currentPunished]]
                      : 'Ошибка :('}
                  </div>
    ...
    <div className={`_PunishedPlayers`}>
      <div className="switch-and-search">
        <div className="switch">
          ...
          />
          <div className="name">
            {CategoriesPunishedNames[Categories[currentPunished]]}
          </div>
          <div
        ...
        <div
          className={`input-container ${isActiveSearch && '-active'}`}
          onClick={handleClick}
        >
          <input
            value={inputValue}
            ...
            style={{
              paddingLeft: `${isActiveSearch ? '1.1111vh' : '3.7036vh'}`,
            }}
