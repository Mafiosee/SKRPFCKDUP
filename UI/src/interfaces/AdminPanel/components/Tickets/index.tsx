...
export const Tickets: React.FC = () => {
  ...
  const [typeFilter, setTypeFilter] = useState<ReportReason | null>(null)
  ...
  const [currentFastAnswerId, setCurrentFastAnswerId] = useState<number | null>(
    null,
  )
    ...
    (event: KeyboardEvent) => {
      switch (event.keyCode) {
        case KeyCodes.Enter: {
          ...
          const payload: AdminPanelPayloads[AdminPanelEvents.SendTicketMessage] =
            { id: ticketContent.id, message }
  ...
  const handleClickFilterArrow = (direction: ArrowDirection) => {
    ...
    if (statusFilterIndex < 0) {
      setStatusFilter(
        StatusFilterOrder.at(statusFilterIndex) ?? StatusFilter.Private,
      )
    } else {
      setStatusFilter(
        StatusFilterOrder[statusFilterIndex % StatusFilterOrder.length],
      )
    }
  }
  ...
  const onClickTicket = (ticket: TicketType) => {
    const payload: AdminPanelPayloads[AdminPanelEvents.OpenTicket] = {
      id: ticket.id,
    }
    ...
    ticketContent?.isClosed
      ...
      : buttons.map((btn, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`tool-tip-container ${toolTipId === idx && '-show'}`}
              >
  ...
  const onClickTakeFreeTicket = (ticketId: number) => {
    const payload: AdminPanelPayloads[AdminPanelEvents.TakeTicket] = {
      id: ticketId,
    }
          ...
          <div key={idx} className={'message'} ref={lastMessageRef}>
            ...
            <div className="content">
              <div className="other">
                ...
                <span>{thisMessage.senderName}</span>{' '}
                <span>{thisMessage.date}</span>
              </div>
  ...
  const onClickDeleteFastAnswer = (id: number) => {
    const payload: AdminPanelPayloads[AdminPanelEvents.DeleteFastAnswer] = {
      id,
    }
            ...
            <div className="content">
              {fastAnswers.length <= 0 ? (
                <div className={`not-find-answers`}>
                  <div className={'text'}>
                    У вас ещё нет быстрых ответов, создайте первый
                  </div>
                    ...
                    {fastAnswers.map((answer, idx) => (
                      <div key={idx} className={'answer-container'}>
                        ...
                        <div className="btns">
                          <div
                            className={`edit`}
                            onClick={() => onClickEditFastAnswer(idx)}
                          />
                          <div
                            className={`delete`}
              ...
              <div className="content">
                <div className="blocks">
                  ...
                  <div className={`block reason`}>
                    ...
                    <div className="text">
                      {ticketContent.ticketInfo.description}
                    </div>
                  </div>
                    ...
                    <div className={`block proofs`}>
                      ...
                      <div className="text">
                        {ticketContent.ticketInfo
                          ? ticketContent.ticketInfo.proofs.map(
                              (proof, idx) => (
  ...
  return (
    <div className={'_Tickets'}>
      <div className="tickets-list">
        ...
        <div
          className={`select ${openedTypeSelect && '-opened'}`}
          onClick={() => setOpenedTypeSelect((prev) => !prev)}
        >
          <div className="current">
            {typeFilter == null ? 'Все' : TypeName[typeFilter]} (
            {typeFilter == null
              ? tickets.length
              : ticketsByStatusAndType[typeFilter].length}
            )
          </div>
          <div className="list">
            {[null, ...TypeOrder].map((type) => (
              <div
                key={type}
                className="item"
                onClick={() => setTypeFilter(type)}
              >
                {type == null ? 'Все' : TypeName[type]} (
                {type == null
                  ? tickets.length
                  : ticketsByStatusAndType[type].length}
                )
              </div>
            ))}
          </div>
          ...
          {(typeFilter == null
            ...
            >
              <div className="sender">
                {ticket.senderInfo.name} #{ticket.senderInfo.uid}{' '}
                {ticket.senderInfo?.id ? `[${ticket.senderInfo.id}]` : ''}
              </div>
              ...
              {ticket?.amountNotification != null &&
                ticket.amountNotification > 0 && (
                  <div className="amount-messages">
                    {ticket.amountNotification}
                  </div>
                )}
            </div>
          ))}
      ...
      {ticketContent && adminInfo ? (
        <>
          <div className={`ticket-content ${ticketContent && '-show'}`}>
            ...
            {ticketContent.suspectInfo != null && (
              <div className="suspect">
                <div className="info">
                  <div className="row">
                    <div className="block">
                      <div className="title">Нарушитель:</div>
                      <div className="value">
                        {ticketContent.suspectInfo.name}
                      </div>
                    </div>
                    ...
                    {ticketContent.suspectInfo.id !== null && ticketContent.suspectInfo.id !== undefined && (
                      <>
                        <div className="separator" />
                        <div className="block">
                          <div className="title">ID:</div>
                          <div className="value">
                            {ticketContent.suspectInfo.id}
                          </div>
                        </div>
                      </>
                    )}
                ...
                {ticketContent.suspectInfo.id !== null && ticketContent.suspectInfo.id !== undefined && (
                  <div
                    className="recon"
                    onClick={() => {
                      const payload: AdminPanelPayloads[AdminPanelEvents.ReconPlayer] =
                        { uid: ticketContent?.suspectInfo?.uid ?? '' }
                      callClient(AdminPanelEvents.ReconPlayer, payload)
                    }}
                  />
                )}
              </div>
            ...
            <div className="content">
              <div className="num-container">
                <div className="line" />
                <div className="num">
                  Тикет №{ticketContent && ticketContent.id}
                </div>
                <div className="line" />
              </div>

              <div className="messages">
                {ticketContent && getMessages(ticketContent.messages)}
              </div>
              ...
              {!ticketContent.isPrivate && !ticketContent.isClosed && (
                <div className={'free-ticket'}>
                  <div className="icon" />
                  <div className="text">
                    Заберите тикет, чтобы ответить на него
                  </div>
                  <div
                    className="btn"
                    onClick={() => onClickTakeFreeTicket(ticketContent.id)}
                  >
                    Забрать тикет
                  </div>
                </div>
              ...
              {!ticketContent.isClosed && ticketContent.isPrivate && (
                <div className={'chat-container'}>
                  <div
                    className="fast-answer"
                    onClick={() => setShowFastAnswer(!showFastAnswer)}
                  >
                  ...
                  <div
                    className={`answers-blocks ${showFastAnswer && '-show'}`}
                  >
                    {fastAnswers.map((fastAnswer, idx) => (
                      <div
                        key={idx}
                        className={'answer'}
                        onClick={() => onClickFastAnswer(idx)}
                      >
                        {fastAnswer.name}
                      </div>
                    ))}
