...
type Props = {
  setOpenedProductId: (productId: any) => void
}

const PageStore: React.FC<Props> = ({ setOpenedProductId }) => {
  const {
    tab,
    storePage: { categories, products },
  } = useAppSelector((state) => state.donateStore)
  const [activeCategoryId, setActiveCategoryId] = useState(null)
  const [search, setSearch] = useState('')

  const isOpen = useMemo(() => tab === Tab.Store, [tab])
    ...
    products
      ...
      .map((product, idx) => (
        <Product
          ...
          isLarge={idx === 0 || idx === 5}
          info={product}
          setOpenedId={setOpenedProductId}
        />
      ))
    ...
    categories.map((category, idx) => {
      ...
      return (
        ...
        >
          ...
          <div className="list">{getProducts(category.id)}</div>
        </div>
      )
    })
