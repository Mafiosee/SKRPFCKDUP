...
const Binder: React.FC = () => {
	...
	useEffect(() => {
		const bindsRaw = [
			...escMenu.settings[SettingsTabId.Sound].map(section => section.parameters.filter(parameter => parameter.type === SettingParameterType.Bind)),
			...escMenu.settings[SettingsTabId.Keyboard].map(section => section.parameters.filter(parameter => parameter.type === SettingParameterType.Bind)),
			...escMenu.settings[SettingsTabId.Graphics].map(section => section.parameters.filter(parameter => parameter.type === SettingParameterType.Bind)),
			...escMenu.settings[SettingsTabId.Account].map(section => section.parameters.filter(parameter => parameter.type === SettingParameterType.Bind)),
			...escMenu.settings[SettingsTabId.GameProcess].map(section => section.parameters.filter(parameter => parameter.type === SettingParameterType.Bind)),
		]
		...
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			const bind = binds.find(bind => bind.data.current.codes.join(' ') === keyCodes.join(' '))
			...
			if (bind && bind.data?.closeInterfaceEvent && openedStates?.[bind.data?.id]) {
				try {
	...
	}, [activeModifier, adminPanel.isOpen, animations.isOpen, characterMenu.isOpen, escMenu.isOpen, escMenu.settings, fraction.isOpen, inventory.isOpen, radialMenu.isOpen])
