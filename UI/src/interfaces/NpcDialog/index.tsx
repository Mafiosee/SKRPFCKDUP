...
const NpcDialog = () => {
  ...
  const handleKeyUp = useCallback(
    (event: KeyboardEvent) => {
      ...
      switch (event.keyCode) {
        case KeyCodes.Esc: {
          const closeEvent =
            mainButtons.find((button) => button.icon === 'exit')?.event ?? ''
          const payload: ClickButtonPayload = { event: closeEvent }
          callClient(NpcDialogEvents.ClickButton, payload)
          break
        }
      }
    },
  ...
  const onClickBtn = (btnType: ButtonCategories, event: string) => {
    if (btnType === ButtonCategories.common && commonBtnsRef.current != null) {
      if (
