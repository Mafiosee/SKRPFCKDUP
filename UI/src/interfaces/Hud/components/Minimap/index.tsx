...
export const Minimap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setMapOffset({
      x:
        (4096 * (window.screen.height / 1080) * mapScale) / 2 +
        getFormattedCoordinateByCenter(playerPosition.x) * mapScale -
        containerRect.width / 2,
      y:
        (4096 * (window.screen.height / 1080) * mapScale) / 2 +
        getFormattedCoordinateByCenter(-playerPosition.y) * mapScale -
  ...
  const playerPointPosition = useMemo(
    () => ({
      top:
        ...
        getFormattedCoordinateByCenter(-playerPosition.y),
      left:
        (4096 * (window.screen.height / 1080)) / 2 +
        getFormattedCoordinateByCenter(playerPosition.x),
    }),
    [playerPosition.x, playerPosition.y],
  )

  const getFormattedPointPosition = useCallback(
    (position: Coordinates, alwaysVisible = false) => {
      ...
      if (alwaysVisible) {
        ...
        const distanceByPlayer = getFormattedCoordinateByCenter(
          Math.sqrt(
    ...
    [
      playerPointPosition.left,
      playerPointPosition.top,
      playerPosition.x,
      playerPosition.y,
  ...
  const blipPositions = useMemo(() => {
    ...
    blips.forEach(
      (blip) =>
        (positions[blip.id] = getFormattedPointPosition(
          blip.position,
          blip?.alwaysVisibleOnMinimap,
        )),
    )
    if (marker) {
  ...
  const mapBlips = useMemo(
    () =>
      [...blips, marker].map((blip) => {
        if (!blip) {
          return null
        }

        const distanceByPlayer = Math.sqrt(
          Math.pow(blip.position.x - playerPosition.x, 2) +
            Math.pow(blip.position.y - playerPosition.y, 2),
        )

        const isHidden =
          !blip.alwaysVisibleOnMinimap && distanceByPlayer > 15000
  ...
  return visibleMinimap && type === MapType.Global ? (
      ...
      <div className="ref" ref={containerRef}>
        ...
        >
          ...
          <div
            className="player"
            style={{
              transform: `translate(-50%, -50%) rotate(${-playerHeading}deg)`,
            }}
          />
          <div className="cover" />
          <div className="north" />
