...
const TextTemplate: React.FC<Props> = ({ component }) => {
  const formattedText = useMemo(() => {
    ...
    while (
      ...
      component.template.length -
        paramsOrder.join('').length -
        paramsOrder.length * 5
    ) {
      if (currentParam === null) {
        ...
        let text = component.template.slice(
        ...
        if (component.params[currentParam]?.numberSeparator != null) {
          ...
          if (!isNaN(intText)) {
            text = numberWithSeparator(
              intText,
              component.params[currentParam].numberSeparator ?? '',
            )
          }
        }
        result.push(
          <span
            ...
            style={{ color: component.params[currentParam]?.color }}
          >
            {text}
          </span>,
        )
  ...
  return (
    <div
      className={`_TextTemplate ${component?.isUpperCase && '-uppercase'}`}
