...
import {
  ...
  GridType,
  Size,
} from '../../../../shared/inventory/inventoryType'
import {
  ActionsList,
  cellCurrentSize,
  cellSize,
...
import { getAvailablePosition } from '../../utils/getAvailablePosition'
...
const Grid: React.FC<PropsType> = ({
  ...
  const { equipment, hasContainer, container, inventory } = useAppSelector(
  ...
  const [playItemInteractionDrinks] = useSound(
    require('../../../../assets/sounds/item-interaction_drinks.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionFood] = useSound(
    require('../../../../assets/sounds/item-interaction_food.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionWeapon] = useSound(
    require('../../../../assets/sounds/item-interaction_weapon.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionArmor] = useSound(
    require('../../../../assets/sounds/item-interaction_armor.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionPotions] = useSound(
    require('../../../../assets/sounds/item-interaction_potions.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionClothes] = useSound(
    require('../../../../assets/sounds/item-interaction_clothes.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionManuscripts] = useSound(
    require('../../../../assets/sounds/item-interaction_manuscripts.mp3'),
    { volume: sfxBase },
  )
  const [playItemInteractionResources] = useSound(
    require('../../../../assets/sounds/item-interaction_resources.mp3'),
    { volume: sfxBase },
  )
  ...
  useEffect(() => {
    ...
    const item = isOutOfGrid
      ? undefined
      : items.find(
          (item) =>
            item.id ===
            cellsState[drag.info.hoveredCell.y][drag.info.hoveredCell.x]
              ?.itemId,
        )
    // @ts-expect-error qwe
    if (
      item != null &&
      drag.info.itemId != null &&
      item.id !== drag.info.itemId &&
      item.itemHash &&
      drag.info.item?.itemHash &&
      item.itemHash === drag.info.item?.itemHash
    ) {
      for (
    ...
    for (let y = startCellPosition.y; y < endCellPosition.y; y++) {
      ...
      for (let x = startCellPosition.x; x < endCellPosition.x; x++) {
        ...
        const hasItem =
          cells[y][x]?.itemId != null && cells[y][x].itemId !== drag.info.itemId
        if (hasItem) {
          hasCollision = true
        }
        cellsToUpdate.push({
          ...
          {new Array(size.width).fill(null).map((_, colIdx) => {
            ...
            return (
              <div
                ...
                onMouseEnter={() => {
                  if (!drag || noInfo || gridId === Grids.Search) {
                    return
                  }
                  drag.set({
                    ...drag.info,
                ...
                onMouseUp={() => {
                  ...
                  if (drag.info.gridId === Grids.Equipment) {
                    ...
                    if (
                      data.from.gridId === Grids.Nearby &&
                      data.to.gridId === Grids.Nearby
                    ) {
                      return
                    }
      ...
      return (
        <div
          ...
          onMouseDown={(event: React.MouseEvent<HTMLDivElement>) => {
            if (isPressedShift || isPressedControl || gridId === Grids.Search) {
              return
            }
          ...
          onClick={() => {
            if (isPressedShift) {
              ...
              return callClient(InventoryEvents.HalfSplitItem, payload)
            }
            if (isPressedControl) {
              let requiredGrid:
                | { id: Grids; size: Size; items: ItemDto[] }
                | undefined
              switch (gridId) {
                case Grids.Nearby:
                case Grids.Container: {
                  requiredGrid = {
                    id: Grids.Inventory,
                    size: inventory.size,
                ...
                case Grids.Inventory: {
                  ...
                  if (
                    trade.isOpen &&
                    trade.confirmState === ConfirmStates.Wait &&
                    !trade.isAccepted
                  ) {
                    requiredGrid = {
                      id: Grids.TradeGive,
                      size: trade.give.size,
                      items: trade.give.items,
                    }
                    break
                  }
                  break
                }
                case Grids.TradeGive: {
                  if (
                    trade.isOpen &&
                    trade.confirmState === ConfirmStates.Wait &&
                    !trade.isAccepted
              ...
              const foundPosition = getAvailablePosition(
                item,
                requiredGrid.size,
                requiredGrid.items,
              )
              ...
              const data: MoveItemData = {
                itemId: item.id,
                isTurned: foundPosition.isTurned,
                from: {
                  gridId,
                  position: item.position,
                },
                to: {
                  gridId: requiredGrid.id,
