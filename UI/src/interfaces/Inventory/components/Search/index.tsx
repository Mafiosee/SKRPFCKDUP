...
const Search: React.FC<Props> = ({ drag, hover }) => {
	const { search } = useAppSelector(state => state.inventory)

	return search != null ? (
		<div
			className='_Inventory_Search'>
			<div className='background'>
				<SearchBg race={search.race} />
				<div className='title'>{search.title}</div>
				<div className='close' onClick={() => callClient(InventoryEvents.Close)} />
			</div>
				...
				<div className='body'>
					<div className='header'>
						...
						<div className='weight'>
							...
							<div
								className='value'>{search.grid.items.reduce((acc, item) => acc + (item?.weight ?? 0) * (item?.amount ?? 1), 0).toFixed(2)} кг
							</div>
						</div>
					</div>
					<div className='grid'>
						<Grid grid={search.grid} gridId={Grids.Search} drag={drag} hover={hover} maxHeight={8} />
					</div>
				</div>
			</div>
		</div>
