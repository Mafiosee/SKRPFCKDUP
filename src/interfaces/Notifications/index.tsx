...
const Notifications: React.FC = () => {
	...
	const getFormattedTemplate = (template: string, params: TextTemplateParams) => {
		...
		while (symbolIndex < template.length - paramsOrder.join('').length - paramsOrder.length * 5) {
			if (currentParam === null) {
				...
				result.push(
					<span
						key={symbolIndex}>{template.slice(symbolIndex, currentParam == null ? template.length : paramsInfo[currentParam].openTagIndex)}</span>)
				if (currentParam) {
					symbolIndex = paramsInfo[currentParam].openTagIndex + currentParam.length + 2
				} else {
					break
				}
			} else {
				let text = template.slice(paramsInfo[currentParam].openTagIndex + currentParam.length + 2, paramsInfo[currentParam].closeTagIndex)
				if (params[currentParam]?.numberSeparator != null) {
					const intText = parseInt(text)
					if (!isNaN(intText)) {
						text = numberWithSeparator(intText, params[currentParam].numberSeparator ?? '')
					}
				}
				result.push(
					<span
						...
						style={{ color: params[currentParam]?.color }}>{text}</span>)
					...
					return (
						...
						>
							...
							<div className='content'>
								...
								{template ? <div className='text'>{getFormattedTemplate(template, params)}</div> : null}
							</div>
						</div>
					)
				})}
