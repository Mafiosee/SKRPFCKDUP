...
import { FractionEvents, FractionSetWarehouseIsOpenPayload } from '../../../../shared/Fraction/events'
...
const PageBank = () => {
	...
	const [activeFilterIds, setActiveFilterIds] = useState<any[]>([LogType.Take, LogType.Put, LogType.Tax])
	...
	const renderList = () => logs.map(({ datetime, action, type }, index) => {
		...
		<div className='_PageBank'>
			...
			<div className='block -warehouse'>
				...
				<div className='switch'>
					<div className={`item ${isWarehouseOpen && '-active'}`} onClick={() => {
						const payload: FractionSetWarehouseIsOpenPayload = { isOpen: true }
					...
					<div className={`item ${!isWarehouseOpen && '-active'}`} onClick={() => {
						const payload: FractionSetWarehouseIsOpenPayload = { isOpen: false }
						callClient(FractionEvents.SetWarehouseIsOpen, payload)
					}}>
						<div className='background' />
						<div className='active' />
			...
			<div className='logs'>
				<div className='header'>
					...
					<div className='filter' onClick={() => setIsOpenFilters((prev: boolean) => !prev)}>Фильтр</div>
					<div className={`filters ${isOpenFilters && '-opened'}`}>{renderFilters()}</div>
				</div>
