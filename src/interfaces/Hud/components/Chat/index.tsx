					...
					{showMessages && (
						<>
							<div
								...
								style={{ opacity: isOpacity && !showInput ? .5 : 1 }}
								ref={messagesRef}
							>
								{messages.map((message, idx) => (
									<div key={idx} className={`message`}>
										<div className='icon' style={{
											backgroundImage: !message?.senderInfo ? ChatStatusIconUrls[message.senderStatus].background : undefined,
										}}>
											<div className='content' style={{
												backgroundImage: message?.senderInfo ? `url(${ChatRaceIconUrls[message.senderInfo.race][message.senderInfo.gender]})` : `url(${ChatStatusIconUrls[message.senderStatus].content})`,
											}} />
										</div>
										<div
											className='sender'
